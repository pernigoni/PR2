...
let rec eval (e : exp) (env : evT env) : evT = (* interprete *)
    ...
    | Iterate (Den funName, arg, n) -> 
        let m = exp_sub n (EInt 1) env in
            let res = Apply (Den funName, arg) in
                if (exp_equals m (EInt 0) env) = true 
...
and exp_sub (n : exp) (m : exp) (env : evT env) : exp = (* Sub non va bene perché restituisce un evT ma l'ultimo parametro di Iterate è un exp *)
    match typecheck IntTag (eval n env), typecheck IntTag (eval m env), n, m with
    | true, true, EInt a, EInt b -> EInt (a - b)
    | _, _, _, _ -> raise TypeError

and exp_equals (v1 : exp) (v2 : exp) (env : evT env) : bool = (* Equals non va bene perché restituisce un evT ma nella guardia dell'if mi serve un bool *)
    match typecheck IntTag (eval v1 env), typecheck IntTag (eval v2 env), v1, v2 with
    | true, true, EInt n, EInt m -> if n = m then true else false
    | _, _, _, _ -> raise TypeError;;